...
void setpixel(uint8_t *buff, uint8_t x, uint8_t y, uint8_t color) {
	...
	buff[x + ((y/8)*128) ] = buff[x + ((y/8)*128) ] | _BV(-(y%8)+7);
...
void drawstring(uint8_t *buff, uint8_t x, uint8_t line, char *c) {
	...
	while(*c != '\0')
	{
		for (uint8_t i =0; i<5; i++ )
		{
			buff[x + (line*128) ] = buff[x + (line*128) ] |  pgm_read_byte(font+(*c*5)+i);
			x++;
		}
		write_buffer(buff);
		c++;
...
void drawrect(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color) {
	for(int i=0;i<=w;i++)
	{
		setpixel(buff,x+i,y,0);
		setpixel(buff,x+i,y+h,0);
	}
	for(int j=1;j<=h-1;j++)
	{
		setpixel(buff,x,y+j,0);
		setpixel(buff,x+w,y+j,0);
	}
}
...
void drawcircle(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t r,uint8_t color) {
	 ...
	 // after translation
	 setpixel(buff,x+x0,y+y0,0);
	 // When radius is zero only a single
	 // point will be printed
	 if (r > 0)
	 {
		 setpixel(buff,-x+x0,y+y0,0);
		 setpixel(buff,y+x0,x+y0,0);
		 setpixel(buff,y+x0,-x+y0,0);
	 }
...
void clearcircle(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t r,uint8_t color) {
	...
	// Printing the initial point on the axes
	// after translation
	clearpixel(buff,x+x0,y+y0);
	clear(buff,x0,y0,x,y,0);
...
{
	if((0<x)&(x<63) & (0<y)&(y<32))
	{
		if(pad_c0>=7)
		{
			clearpad(buff,pad_c0,0);
			pad_c0--;
			drawpad(buff,pad_c0,0);
			//_delay_ms(2);
		}
		
	}
	
	 if((0<x)&(x<63) & (63>y)&(y>=32))
	{
		if(pad_c0<=56)
		{
			clearpad(buff,pad_c0,0);
...
{
	if((63<x)&(x<127) & (0<y)&(y<32))
	{
		if(pad_c1>=7)
		{
			clearpad(buff,pad_c1,1);
			pad_c1--;
			drawpad(buff,pad_c1,1);
			//_delay_ms(2);
		}
		
	}
	
	 if((63<x)&(x<127) & (63>y)&(y>=32))
	{
		if(pad_c1<=56)
		{
			clearpad(buff,pad_c1,1);
...
void blink_blue()
{
	...
	DDRD |= 0x00;PORTD &= ~0x00;PORTD |= 0x00;
	DDRB |= 0x04;PORTB &= ~0x04;PORTB |= 0x00;
	_delay_ms(100);
	DDRD &= ~0x00;PORTD |= 0x00;PORTD |= 0x00;
	DDRB &= ~0x04;PORTB |= 0x04;PORTB |= 0x00;
	DDRD |= 0x80;PORTD &= ~0x80;PORTD |= 0x00;
...
void blink_white()
{
	...
	DDRB &= ~0x01;PORTB |= 0x01;PORTB |= 0x00;
	DDRD |= 0x80;PORTD &= ~0x80;PORTD |= 0x00;
	DDRB |= 0x05;PORTB &= ~0x05;PORTB |= 0x00;
	_delay_ms(100);
	DDRD &= ~0x80;PORTD |= 0x80;PORTD |= 0x00;
	DDRB &= ~0x05;PORTB |= 0x05;PORTB |= 0x00;
	DDRD |= 0x80;PORTD &= ~0x80;PORTD |= 0x00;
